...
int processStrings(struct dataHeader *header, int mode){
	int i, j, lineFlag=0, tagFlag=0, strLength=0;

	/*MYSQL Vars definition*/
	MYSQL mysql;
	char query[MAX_QUERY], sizeStr[8], *dbTok, encoded[5000]= "", cleanCoded[5005] = "";
	int size;
	char insertStr[4000] = "insert into files values('";
	...
	char readPipeBuff[4096], nextTemp[500], exFile[70];
	temp = header->next;
	header->length = 0;
	...
	if(pid == 0){
		...
		if(fp == NULL){
			...
			if(mode == 1){
				...
				*/
				...
				strcat(query, "create table if not exists files (Content TEXT not null,");
        		strcat(query, "Length int,");
		        strcat(query, "Name char(50),");
        		strcat(query, "Timestamp char(25),");
        ...
        //printf("Created tables\n");
				...
				taggedFp = strcat(header->name, ".html");
				strcpy(exFile, taggedFp);
				htmlFp = fopen(taggedFp, "w");
				while(fgets(readPipeBuff, 4096, fp) != NULL){
					strcat(encoded, readPipeBuff);
                    fputs(readPipeBuff, htmlFp);
                }
				...
				size = ftell(htmlFp);
				/*dealing with name*/
				dbName = nameEdit(taggedFp);
				dbTok = strtok(dbName, ".");

				sprintf(sizeStr, "%d" , size);
				mysql_real_escape_string(&mysql, cleanCoded, encoded, strlen(encoded));
				strcat(insertStr, cleanCoded);
				strcat(insertStr, "', ");
				strcat(insertStr, sizeStr);
    			strcat(insertStr, ", '");
    			strcat(insertStr, dbTok);
    			strcat(insertStr, "', ");
   				strcat(insertStr, "NOW())");

				if(mysql_query(&mysql,  insertStr)){
                    printf("Falure\n");
